...
.INPUTS
    ...
    $gpname - name of the group policy to link
    $target - the ou that the gp will be linked to in this format "OU=Districts,OU=IT,dc=enterprise,dc=com"
...
Try {
  New-GPLink -Name $gpname -Target $target -LinkEnabled $linkenabled -Enforced $enforced
}
