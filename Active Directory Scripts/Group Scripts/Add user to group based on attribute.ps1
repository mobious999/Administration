<#
.SYNOPSIS
  This script can be used to add users to a group based on attribute being populated
.DESCRIPTION
...
Try { 
  Get-ADUser -SearchBase 'DC=Quinn,DC=local' -filter {employeeNumber -ne "$null"} | out-file c:\users.txt
  Send-MailMessage -To "admin@domain.com" -From "script@domain.com" -subject "users added to group" -SmtpServer "smtpserver.quinn.local" -attachments "c:\users.txt"
}
