<#
.SYNOPSIS
  This script can be used to rename an ou
.DESCRIPTION
...
.EXAMPLE
  .\renameou -oldname (insername) -newname (insert name)
...
Try {
  Get-ADOrganizationalUnit -Filter "Name -eq $oldname" | Rename-ADObject -NewName $newname
}
