...
.EXAMPLE
  .\export gpo -exportpath (path for report) -xmlfilename (name of xml file) -htmlfilename (name of htmlfile)
...
#If export path is populated the directory will be created otherwise the current directory will be created
if ($exportpath){
...
Try {
  ...
  if (!exportpath){
    if (!$htmlfilename){
      Get-GPOReport -All -ReportType Xml -Path $scriptdir\$xmlfilename
    }
    elseif (!$xmlfilename){}
      Get-GPOReport -All -ReportType Xml -Path $scriptdir\$xmlfilename
    }
  {  
    ...
    Get-GPOReport -All -ReportType Html -Path  $exportpath\$htmlfilename
    Get-GPOReport -All -ReportType Xml -Path $exportpath\$xmlfilename
  }  
}
